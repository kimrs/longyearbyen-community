...
export default function GeneralDetailPage() {
  ...
  const { generalPosts, currentUser, addComment, setShowLoginModal } = useApp();

  const post = generalPosts.find((p) => p.id === postId);
  ...
  if (!post) {
    return (
      <div className="flex flex-col items-center gap-4 py-12">
        ...
        <Button variant="secondary" onClick={() => navigate('/oppslagstavla')}>
          Tilbake til Oppslagstavla
        </Button>
      </div>
    );
  }
  ...
  const handleComment = () => {
    ...
    addComment('oppslagstavla', post.id, {
  ...
  return (
    <div className="flex flex-col pb-4">
      ...
      <button
        type="button"
        onClick={() => navigate('/oppslagstavla')}
